...
inline int gpuDeviceInit(int devID) {
    ...
    Check(cudaDeviceGetAttribute(&computeMode, cudaDevAttrComputeMode, devID));
    Check(cudaDeviceGetAttribute(&major, cudaDevAttrComputeCapabilityMajor, devID));
    Check(cudaDeviceGetAttribute(&minor, cudaDevAttrComputeCapabilityMinor, devID));
...
inline int gpuGetMaxGflopsDeviceId() {
    ...
    while (current_device < device_count) {
        int computeMode = -1, major = 0, minor = 0;
        Check(cudaDeviceGetAttribute(&computeMode, cudaDevAttrComputeMode, current_device));
        Check(cudaDeviceGetAttribute(&major, cudaDevAttrComputeCapabilityMajor, current_device));
        Check(cudaDeviceGetAttribute(&minor, cudaDevAttrComputeCapabilityMinor, current_device));
        ...
        if (computeMode != cudaComputeModeProhibited) {
            ...
            Check(cudaDeviceGetAttribute(&multiProcessorCount, cudaDevAttrMultiProcessorCount, current_device));
...
inline int findIntegratedGPU() {
    ...
    while (current_device < device_count) {
        ...
        Check(cudaDeviceGetAttribute(&computeMode, cudaDevAttrComputeMode, current_device));
        Check(cudaDeviceGetAttribute(&integrated, cudaDevAttrIntegrated, current_device));
        // If GPU is integrated and is not running on Compute Mode prohibited,
        // then cuda can map to GLES resource
        if (integrated && (computeMode != cudaComputeModeProhibited)) {
            Check(cudaSetDevice(current_device));
...
inline bool checkCudaCapabilities(int major_version, int minor_version) {
    ...
    Check(cudaDeviceGetAttribute(&minor, cudaDevAttrComputeCapabilityMinor, dev));
